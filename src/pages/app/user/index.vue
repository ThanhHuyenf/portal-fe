...
const openUserListDialog = (type, user ) => {
  userListDialog.value.open(type, user)
}
</script>
<template>
  <MainContainer one-screen title="Danh sách người dùng">
    ...
    <el-table v-loading="state.loading" :data="state.data" class-name="mt-10" row-key="devices_id">
      ...
      <el-table-column :label="$t('device.device_status')" prop="status">
      ...
      <el-table-column fixed="right" label="Actions" width="120">
        <template #default="{ row }">
          ...
          <my-icon class="ml-10 cursor-pointer" color="primary" name="ic:baseline-delete" size="20px"
                   @click="onDelete(row)"/>
          <my-icon class="ml-10 cursor-pointer" color="primary" name="mdi:eye" size="20px"
                   @click="onDelete(row)"/>
        </template>
      </el-table-column>
    </el-table>
