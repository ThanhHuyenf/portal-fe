...
<template>
  ...
  <el-row align="middle" class="layout-header" tag="header">
    <LayoutLogo v-if="topMenu || !state.menuList.length"/>

    <LayoutMenu v-if="topMenu" mode="horizontal"/>

    <div :class="{ 'flex-1': !topMenu }" class="layout-header__r">
      <!-- 从右向左，详见样式 flex-direction -->
      <el-menu mode="horizontal">
        <el-sub-menu :teleported="true" index="1" popper-class="layout-header__menu">
          <template #title>
            <el-row align="middle">
              <el-avatar
                  :size="36"
